...
static void generate_native_shadow(float opacity, unsigned long* out_data) {
    ...
    out_data[4] = create_slice(full, cx0, cy0 + th, tw, pb);          /* Bottom */
...
static void add_shadow(Window toplevel, Window client, int x, int y, int w, int h) {
    ...
    if (use_native_shadows) {
        ...
        XChangeProperty(dpy, client, A_KDE_NET_WM_SHADOW, XA_CARDINAL, 32, PropModeReplace, (unsigned char*) data, 12);
    } else {
...
static void usage(const char* prog) {
    fprintf(stderr,
        ...
        "\n"
        "Options:\n"
        "  --radius  N     Shadow blur radius in pixels  (default: 50)\n"
        "  --opacity F     Shadow opacity 0.0-1.0        (default: 0.80)\n"
        "  --offset-x N    Horizontal shadow offset      (default: 0)\n"
        "  --offset-y N    Vertical shadow offset        (default: 10)\n"
                    ...
                    for (ShadowEntry* e = shadow_list; e; e = e->next) {
                        if (e->toplevel == active || e->client == active) {
                            if (!e->is_active) {
                                e->is_active = 1;
                                if (use_native_shadows) {
                                    XChangeProperty(dpy, e->client, A_KDE_NET_WM_SHADOW, XA_CARDINAL, 32, PropModeReplace, (unsigned char*) native_active_data, 12);
                                } else {
                                    render_shadow(e, e->w, e->h);
                                    XClearWindow(dpy, e->shadow);
                                }
                            ...
                            if (e->is_active) {
                                e->is_active = 0;
                                if (use_native_shadows) {
                                    XChangeProperty(dpy, e->client, A_KDE_NET_WM_SHADOW, XA_CARDINAL, 32, PropModeReplace, (unsigned char*) native_inactive_data, 12);
                                } else {
                                    render_shadow(e, e->w, e->h);
                                    XClearWindow(dpy, e->shadow);
                                }
